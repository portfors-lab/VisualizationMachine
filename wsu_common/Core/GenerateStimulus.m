...
for test_num = test_nums
            ...
            if ~isempty(stimulus)
                ...
                for istim = 1:length(stimulus)
                    ...
                    if ~exist([prefs.audio_directory stim.vocal_call_file], 'file')
                        %search subfolders for audiofile
                        subfolders = textscan(genpath(prefs.audio_directory), '%s', 'delimiter', ':');
                        subfolders = subfolders{1};
                        audioDir = [];
                        for isubf = 1:length(subfolders)
                            if exist([subfolders{isubf} filesep stim.vocal_call_file], 'file')
                                audioDir = [subfolders{isubf} filesep];
                                break;
                            end
                        end
                        if isempty(audioDir)
    ...
    %                     error(['Stimulus file ' prefs.audio_directory stim.vocal_call_file ' not found']);
                        ...
                        audioDir = prefs.audio_directory;
                    end
                    [vocalization sampling_frequency] = ParseAudioData([audioDir stim.vocal_call_file]);
                    ...
                    stimulus_signal(istim,:) = zeros(1,ceil(stimulus_length * sampling_frequency));
    ...
    %                 experiment_data.test(test_num).trace(trace_num).stimulus(1).duration = vocal_length;
                    ...
                    individual_stimulus_signals{test_num,trace_num,istim} = vocalization;
                    %Add this signal to the combined stimulus signal
                    stimulus_signal(istim,stim_offset:(stim_offset+stim_samples-1)) = ...
                                    stimulus_signal(istim, stim_offset:(stim_offset+stim_samples-1)) + vocalization;
                end
            else
        ...
        elseif strcmp(test.testtype,'fmsweep')
            continue
            %do nothing
        else
