function [unique_frequencies, ...
          unique_attenuations, ...
          test_data, ...
          tune_classes, hands] = VisualizeTestData(experiment_data, ...
                                            prefs, ...
                                            test_nums, ...
                                            plot_flag, ...
                                            output_path, ...
...
% added differing spike peak thresholds 11/12

warnColor = 'orange';
hands = []; %getting out some axes handles - 11/12
...
if calculate_spike_times        
    raw_data = ExtractRawData(experiment_data,prefs,tests_2_process,[]);


    if length(prefs.spike_time_peak_threshold) > 1
        thresholds = prefs.spike_time_peak_threshold;
    else
        thresholds = [];
    end
    [spike_times ...
...
%                     output_path = [output_path ' ' prefs.colormap_name];
            ...
            [X Y] = calibrateSpeakers(unique_frequencies,unique_attenuations,prefs.speaker_calibration);
            if pf(plot_surface)
                if exist('output_path','var') && ~isempty(output_path)
                    if ~strcmp(prefs.colormap_name, 'jet')
%                         output_path = [output_path ' ' prefs.colormap_name];
...
%                         output_path = [output_path ' ' prefs.colormap_name];
                ...
                [~, hand] = contourf(X,Y,test_data');
                hands(end+1) = get(hand, 'parent');
                view(2);
                if prefs.invert_color
                    colormap(flipud(prefs.colormap));
                else
