...
if exist(prefs.raw_data_filepath, 'file')
...
if exist(prefs.hdf5_data_filepath, 'file')
    ...
    for test_num = test_nums
        filepathname = prefs.hdf5_data_filepath;
        test_data = hdf5read(filepathname, experiment_data.test(test_num).testname);
        [samples, channels, reps, traces] = size(test_data);
    
        display(experiment_data.test(test_num).testname)
        
        for trace_num = 1:traces
    
            trace = zeros(reps,samples);
    
            for rep_num = 1:reps
               for sample_num = 1:samples
                    trace(rep_num, sample_num) = test_data(sample_num, 1, rep_num, trace_num);
                end
            end
    
            raw_data{test_num, trace_num} = trace;
