...
for test_num = test_nums
    ...
    for trace_num = traces_2_process
        ...
        for istim = 1:length(trace.stimulus)
            stimulus_signal = stimulus_signals{test_num,trace_num};
            if ~isempty(stimulus_signal)
                stimulus_signal = stimulus_signal(istim,:);
                ...
                if var(stimulus_signal) ~= 0 && ~isempty(stimulus_signal)
        ...
        %         S = S.*repmat(voltage_to_pressure_conversion_coefficients(:),1,length(times));
                ...
                out_stimulus_spectrograms{test_num,trace_num,istim} = S;
                out_stimulus_spectrogram_time_idxs{test_num,trace_num,istim} = times;
                out_stimulus_spectrogram_freq_idxs{test_num,trace_num,istim} = frequencies;
                out_stimulus_dB_SPLs{test_num,trace_num,istim} = dB_SPL;
            end
        end
    end
end
