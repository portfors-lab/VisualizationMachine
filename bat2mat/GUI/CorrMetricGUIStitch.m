...
%Joel U Oct 2015, adapted from CorrMetricGUI.m
...
uicontrol(fh, 'style', 'text', 'position',  [85 395 250 30], 'string', 'Correlation Matrix Stitch', 'fontsize', 14, 'backgroundcolor', bcolor);
...
uicontrol(fh, 'style', 'text', 'position', [15 255 100 20], 'string', 'Stitch numbers:', 'horizontalalignment', 'left', 'backgroundcolor', bcolor);
testNumBox =  uicontrol(fh, 'style', 'edit', 'position', [150 275 150 20], 'callback', @resetNums);
testNumBox2 =  uicontrol(fh, 'style', 'edit', 'position', [150 255 150 20], 'callback', @resetNums);
...
bgh = uibuttongroup('parent', fh,  'units', 'pixels','position', [15 160 400 75], 'backgroundcolor', bcolor, 'selectionchangefcn', @selectionFun);
...
thresholds = [];
    ...
    function okFun(hObj,eventData)
        ...
        stitch_nums = str2num(get(testNumBox2, 'string'));
        
        if isempty(test_nums)
            errordlg('! Must select tests to correleate');
            return
        end
        if isempty(stitch_nums)
            errordlg('! Must select tests to stitch');
            return
        end
        ...
        if selectedRadio == universalRadio
           ...
           if isempty(thresholds) 
               thresholds = ones(length(test_nums)+1)*str2double(get(threshBox, 'string'));
           end
        end
        ...
        % Name for the stitched test to use for plot
        vocalName = 'StitchedTests';
        for num = 1:numel(stitch_nums)
            if num == 1
                vocalName = [vocalName '-' num2str(stitch_nums(num))];
            else
                vocalName = [vocalName '&' num2str(stitch_nums(num))];
            end
        end
        ...
        if calculate_spike_rates
            ...
            % Get spike times for stitched numbers
            stitchedSpikeTimes = CalculateSpikeTimes(experiment_data, prefs, stitch_nums, trace_num, [], thresholds);
            ...
            display(stitchedSpikeTimes)
            
            %display(length(stitchedSpikeTimes))
            
            % Get the max length of the individual stitchSpikeTimes
            spikeTimeLength = 0;
            for time = 1:length(stitchedSpikeTimes)
                if length(stitchedSpikeTimes{time}) > spikeTimeLength
                    spikeTimeLength = length(stitchedSpikeTimes{time});
                end
            end
            
            % Stitch the spike times together
            [stitchedCell{1:spikeTimeLength, 1}] = deal(zeros(0, 1));
            %display(stitchedCell)
            for time = 1:length(stitchedSpikeTimes)
                if iscell(stitchedSpikeTimes{time})
                    
                    
                    for x = 1:spikeTimeLength
                        temp1 = stitchedCell{x,1};
                        temp2 = stitchedSpikeTimes{time,1}{x,1};
                        ...
                        [size1, ~] = size(temp1);
                        [size2, ~] = size(temp2);
                        % If value already exists, append temp2
                        if (size1 > 1 && size2 > 1) | (temp1 ~= 0 & temp2 ~= 0)
            ...
            % Replace All 0's with zeros(0, 1)
            for item = 1:length(stitchedCell)
                if isequal(stitchedCell{item}, 0)
                    stitchedCell{item} = zeros(0, 1);
                end
            end
            
            display(stitchedCell)
            
            %display(spikeTimeLength)
            
            %display(spikeTimes)
            
            %[numSpikeTimes, ~] = size(spikeTimes);
            spikeTimes{stitch_nums(1),1} = stitchedCell;
            
            %display(size(spikeTimes))
            %display(spikeTimes)
            
            %display(spikeTimes{1,1})
            %display(spikeTimes{2,1})
            %display(spikeTimes{3,1})
            
%            display(spikeTimes{1,1}{2,1})
            
            fprintf('---\n')
            
            %display([test_nums stitch_nums(1)])
            %display(spikeTimes{1,1})
            %display(spikeTimes{2,1})
        ...
        % Append stitch_nums(1) to test_nums
        combine_nums = [test_nums stitch_nums(1)];
        ...
        test_num_idxs = 1:length(combine_nums);
        for test_num1_idx = test_num_idxs
            test_num1 = combine_nums(test_num1_idx);
            for test_num2_idx = 1:test_num1_idx;
                test_num2 = combine_nums(test_num2_idx);
