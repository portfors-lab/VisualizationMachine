function prefs = GeneratePreferencesNew(base_batlab_data_path)
%
...
slash = GetSlash;
if ~isequal(base_batlab_data_path(end), slash)
   base_batlab_data_path = [base_batlab_data_path slash];
end 
rev = fliplr(base_batlab_data_path);
[tok parentDir] = strtok(rev, slash);
tok = fliplr(tok);
parentDir = fliplr(parentDir);

    prefs.extracted_data_folder = [parentDir 'Extracted Data' slash];
    if ~exist(prefs.extracted_data_folder, 'file')
        mkdir(prefs.extracted_data_folder);
    end
    prefs.Bat2Matlab_data_filepath = [parentDir 'Extracted Data' slash tok '.mat'];
    ...
    prefs.output_data_filepath = [prefs.bat2matlab_directory 'Output' slash];
