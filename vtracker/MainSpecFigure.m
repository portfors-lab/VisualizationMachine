...
uimenu(fileMenu, 'label', 'Save Inputs...', 'callback', @saveProj);
...
uimenu(fixMenu, 'label', 'window lines', 'callback', @toggleWlines);
...
cutButton = uicontrol('style', 'pushbutton', 'units', 'normalized', 'position', [0.05 0.1 0.1 0.04], 'string', 'cut', 'tooltipstring', 'remove sections of spectrogram', 'callback', @cutoutSections);
...
%        userInputs.values.dbRange = inputs.values.dbRange;
        if strcmp(wlines, 'on')
            userInputs.wlines = true;
        else
            userInputs.wlines = false;
        end;
...
%         [cmapIdx, ok] = listdlg('liststring', cmapList, 'selectionmode', 'single', 'listsize', [150 105], 'name', 'Select colomap');
    ...
    set(handles.wlines, 'visible', wlines);
    end
end