function [handlesVector, yp] = PlotHarmonicStates(States,varargin)
...
% Last Modified 3/24/11 Amy Boyle
...
plotResiduals = true;
plotParticles = false; %not in estimates structure
plotTrueSignal = true;
...
%             case lower('invertColorMap'),   invertColorMap  = parameterValue;
            ...
            case 'windowlines',                                wlines = parameterValue;
            otherwise,                      error(['Unrecognized property: ' parameterName]);
        end
    end
end
numSteps = sum([plotSyntheticSpectrogram plotSyntheticSpectrogramHarmonics plotSignalCompare plotAmplitudesAndPhases ...
            plotSignalSpectrogram plotSignalSpectrogramHarmonics plotResiduals plotParticles plotTrueSignal]) +2;
if progressNotify
    wb = waitbar(thisStep/numSteps);
    thisStep = thisStep + 1;
end    
%==============================================================================
% Preprocessing
...
% label               = States.label;
if strcmp(stateType, 'Smoothed')
    label = 'Extended Kalman Smoother';
elseif strcmp(stateType, 'Filtered')
...
if plotSignalCompare && ~isempty(y)
    ...
    fh = figure('numbertitle', 'off', 'name', 'Signal Comparison', 'visible', visibility);
    ...
    ylabel('Amplitude');
    if ~isempty(label)
        title(['Signal Comparison For ' label ': NMSE = ' num2str(signalNMSE)]);
    else
         title(['Signal Comparison : NMSE = ' num2str(signalNMSE)]);
    end
    box off;
    AxisSet;
...
if plotAmplitudesAndPhases
    fh = figure('numbertitle', 'off', 'name', 'Amplitudes and Phases', 'visible', visibility);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                         
    if plotSyntheticSpectrogramHarmonics
        ...
        hold off;
        title([label ' Synthsized Signal Harmonic Frequency Estimates']);
%         set(fh, 'name', 'Harmonic Frequency Estimates')
    else
        title([label ' Synthesized Signal']);
    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
%     fh = gcf;
    set(fh, 'name', 'Synthesized Signal')
%     eval(colormapString);
    AxisSet;
    FigureSet;
    if progressNotify
...
if plotResiduals && ~isempty(y)
    ...
    FigureSet;
%     if invertColorMap
%         colormapString = ['colormap(flipud(1-' colorMap '))'];
%     else
%         colormapString = ['colormap(flipud(' colorMap '))'];
%     end
%     eval(colormapString);
    colormap(colorMap)
    if progressNotify
...
if plotParticles && isfield(States,'Particles')
    fh = figure('numbertitle', 'off', 'name', 'Particles', 'Visible', visibility);
...
if plotTrueSignal && ~isempty(trueStates)
    ...
    fh = figure('numbertitle', 'off', 'name', 'Frequency Comparison', 'visible', visibility);
