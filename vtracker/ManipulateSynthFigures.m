function ManipulateSynthFigures(plotHandles, nSections, inputs, stateEstimates, originalSignal)
...
%modified Amy Boyle 3/24/11
    ...
    plotFigure = figure('units', 'normalized', 'position', [0.1 0.1 0.8 0.8], 'menubar', 'none');
...
%             end
%         end     
        AddTabContextMenu(thisTab,'states', stateEstimates, 'inputs', inputs, 'signal', originalSignal);
        close(plotHandle);
    end
    ...
    harmSelectionH = uicontrol(plotFigure, 'style', 'edit', 'units', 'normalized', 'position', [0.2 0.01 boxLength boxHeight], 'tooltipstring', 'which harmonic to apply modifications to (1 = fund, 2=1st harm, etc. empty=all)');
    ...
    randPhaseH = uicontrol(plotFigure, 'style', 'checkbox', 'units', 'normalized', 'position', [0.35  0.05 txtLength+0.05 txtHeight], 'string', 'Randomize Phase','BackgroundColor', bcolor, 'value', 0, 'tooltipstring', 'add a random phase offset to each harmonic');
    revSigH = uicontrol(plotFigure, 'style', 'checkbox', 'units', 'normalized', 'position', [0.35  0.01 txtLength+0.05 txtHeight], 'string', 'Reverse Signal','BackgroundColor', bcolor, 'value', 0, 'tooltipstring', 'synthesize signal backwards');
    
    makeButton = uicontrol(plotFigure, 'style', 'pushbutton', 'units', 'normalized', 'position', [0.8 0.1 boxLength+0.01 boxHeight], 'string', 'write Audio file', 'enable', 'off', 'callback', @createSynthAudio);
    uicontrol(plotFigure, 'style', 'pushbutton', 'units', 'normalized', 'position', [0.8 0.05 boxLength+0.01 boxHeight], 'string', 'generate synth call', 'callback', @viewMorphedSpectrogram, ...
        'tooltipstring', 'apply modifications synthesize');
...
%             end
%         end
        inputs.stateType = modifiers.stateType;
        inputs.sampleRate = stateEstimates.ModelParameters.sampleRate;
        AddTabContextMenu(tab, 'signal', morphedSignal, 'inputs', inputs);
